...
const loginUser = catchAsyncError(async (req, res, next) => {
  ...
  //checking if username or password empty
...
const forgotPassword = catchAsyncError(async(req,res,next)=>{
 ...
 console.log('after save');
   ...
   res.status(200).json({success:true,message:'Reset Password Token generated successfully',resetToken});
})
...
const resetPassword = catchAsyncError(async(req,res,next)=>{
 ...
 const user = await usersModel.findOne({resetPasswordToken,resetPasswordExpires:{$gt:Date.now()}})
...
export { register, loginUser,forgotPassword,resetPassword };
